...
class MonitorGUI(tk.Tk):
    ...
    def _build_summary_tab(self) -> None:
        ...
        for cluster_name, parent_row in cluster_rows:
            ...
            tree.column("farm", width=90, anchor="center")
            tree.column("encolados", width=130, anchor="center")
...
def main() -> None:
    ...
    def _farm_totals_for_cluster(self, cluster_name):
        totals = []
        cluster_name = self._normalize_cluster_name(cluster_name)
        rows_dict = self.current_data.get(cluster_name, {})
        farm_map = CLUSTER_FARM_MAP.get(cluster_name, {})
        for farm_name, node_set in farm_map.items():
            total = 0
            for node_name, row in rows_dict.items():
                node = str(node_name).strip().upper()
                if node in node_set:
                    try:
                        total += int(getattr(row, "encolados", 0) or 0)
                    except Exception:
                        pass
            totals.append((farm_name, total))
        return totals

    def _populate_summary_farm_tables(self):
        for cluster_name in ("Europa", "America", "Brasil", "Aplicaciones"):
            tree = getattr(self, f"summary_farm_tree_{cluster_name.lower()}", None)
            if tree is None:
                continue
            for item in tree.get_children():
                tree.delete(item)
            for farm_name, total in self._farm_totals_for_cluster(cluster_name):
                tree.insert("", "end", values=(farm_name, fmt_num_dot(total)))
