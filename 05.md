...
trim(
    substring(
        ...
        add(indexOf(variables('Notificacion'), 'pedido:'), 8),
        sub(
            indexOf(variables('Notificacion'), '\nelemento'),
            add(indexOf(variables('Notificacion'), 'pedido:'), 8)
        )
    )
)
...
trim(
    substring(
        toLower(variables('Notificacion')),
        add(indexOf(toLower(variables('Notificacion')), 'pedido:'), 8),
        sub(
            indexOf(toLower(variables('Notificacion')), '\nelemento'),
            add(indexOf(toLower(variables('Notificacion')), 'pedido:'), 8)
        )
    )
)
