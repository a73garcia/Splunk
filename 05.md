# 🧩 Power Automate – Notificación Teams “bbb Tarea Asignada”

## 📘 Objetivo
Automatizar la notificación en **Microsoft Teams** al recibir un correo de bbb que informa sobre una tarea asignada a un grupo (por ejemplo: `zzz`).

El mensaje en Teams debe incluir:

- 📬 El **asunto** del correo.  
- 🧩 Los **Detalles Técnicos**.  
- 📁 Los **Detalles Adicionales**.  
- Sin cabeceras ni enlaces, y con formato limpio, en negrita y multilinea.

---

## ⚙️ Estructura del Flujo en Power Automate

### 1️⃣ Disparador
**Acción:** `Cuando llega un nuevo correo electrónico (V3)`  
**Filtro de asunto:**  
```
fue asignada al grupo zzz
```
**Carpeta:**  
```
IT Service Desk
```

---

### 2️⃣ Convertir el cuerpo a texto

**Acción:** `Html to text`  
**Parámetro:**  
```
Contenido = Cuerpo del correo
```

---

### 3️⃣ Variable – TextoPlano
**Acción:** `Inicializar variable`  
- Nombre: `TextoPlano`  
- Tipo: `Cadena`  
- Valor:  
  ```
  body('Html_to_text')
  ```

---

### 4️⃣ Extraer los bloques del correo

**Componer – DetallesTecnicosRaw**
```powerfx
trim(
  split(
    split(variables('TextoPlano'),'DETALLES TÉCNICOS:')[1],
    'DETALLES ADICIONALES:'
  )[0]
)
```

**Componer – DetallesAdicionalesRaw**
```powerfx
trim(
  split(
    split(variables('TextoPlano'),'DETALLES ADICIONALES:')[1],
    'Puede ver'
  )[0]
)
```

---

### 5️⃣ Extraer los valores individuales con control de errores

> Cada expresión usa `if(contains(...), ..., '')` para evitar fallos si el campo no aparece.

#### 🔹 Detalles Técnicos

| Campo | Expresión PowerFx |
|:--|:--|
| **ElemPedido** | `if(contains(outputs('Componer_-_DetallesTecnicosRaw'),'Elemento pedido:'), trim(split(split(outputs('Componer_-_DetallesTecnicosRaw'),'Elemento pedido:')[1],' elemento:')[0]), '')` |
| **Elemento** | `if(contains(outputs('Componer_-_DetallesTecnicosRaw'),' elemento:'), trim(split(split(outputs('Componer_-_DetallesTecnicosRaw'),' elemento:')[1],' Estado:')[0]), '')` |
| **Estado** | `if(contains(outputs('Componer_-_DetallesTecnicosRaw'),' Estado:'), trim(split(split(outputs('Componer_-_DetallesTecnicosRaw'),' Estado:')[1],' Entorno:')[0]), '')` |
| **Entorno** | `if(contains(outputs('Componer_-_DetallesTecnicosRaw'),' Entorno:'), trim(split(split(outputs('Componer_-_DetallesTecnicosRaw'),' Entorno:')[1],' Grupo de asignación:')[0]), '')` |
| **Grupo** | `if(contains(outputs('Componer_-_DetallesTecnicosRaw'),' Grupo de asignación:'), trim(last(split(outputs('Componer_-_DetallesTecnicosRaw'),' Grupo de asignación:'))), '')` |

#### 🔹 Detalles Adicionales

| Campo | Expresión PowerFx |
|:--|:--|
| **Tarea** | `if(contains(outputs('Componer_-_DetallesAdicionalesRaw'),' Tarea de Catálogo:'), trim(split(split(outputs('Componer_-_DetallesAdicionalesRaw'),' Tarea de Catálogo:')[1],' Empresa:')[0]), '')` |
| **Empresa** | `if(contains(outputs('Componer_-_DetallesAdicionalesRaw'),' Empresa:'), trim(split(split(outputs('Componer_-_DetallesAdicionalesRaw'),' Empresa:')[1],' Solicitante:')[0]), '')` |
| **Solicitante** | `if(contains(outputs('Componer_-_DetallesAdicionalesRaw'),' Solicitante:'), trim(split(split(outputs('Componer_-_DetallesAdicionalesRaw'),' Solicitante:')[1],' Abierto:')[0]), '')` |
| **Abierto** | `if(contains(outputs('Componer_-_DetallesAdicionalesRaw'),' Abierto:'), trim(split(split(outputs('Componer_-_DetallesAdicionalesRaw'),' Abierto:')[1],' Fecha de vencimiento:')[0]), '')` |
| **Vencimiento** | `if(contains(outputs('Componer_-_DetallesAdicionalesRaw'),' Fecha de vencimiento:'), trim(last(split(outputs('Componer_-_DetallesAdicionalesRaw'),' Fecha de vencimiento:'))), '')` |

---

### 6️⃣ Publicar mensaje en Teams

**Acción:** `Publicar tarjeta adaptable en un chat o canal (versión preliminar)`  
**Publicar como:** Bot de Flow  
**Chat o canal:** según tu destino (por ejemplo, `1to1 – sss`)  

Pega este JSON completo:

```json
{
  "type": "AdaptiveCard",
  "$schema": "http://adaptivecards.io/schemas/adaptive-card.json",
  "version": "1.5",
  "body": [
    {
      "type": "TextBlock",
      "text": "📬 @{triggerOutputs()?['body/subject']}",
      "wrap": true,
      "size": "Large",
      "weight": "Bolder",
      "color": "Accent"
    },
    {
      "type": "TextBlock",
      "text": "🧩 Detalles Técnicos",
      "wrap": true,
      "weight": "Bolder",
      "spacing": "Medium",
      "separator": true
    },
    {
      "type": "RichTextBlock",
      "isVisible": "@{not(equals(outputs('Componer_-_ElemPedido'), ''))}",
      "inlines": [
        { "type": "TextRun", "text": "Elemento pedido: ", "weight": "Bolder" },
        { "type": "TextRun", "text": "@{outputs('Componer_-_ElemPedido')}" }
      ]
    },
    {
      "type": "RichTextBlock",
      "isVisible": "@{not(equals(outputs('Componer_-_Elemento'), ''))}",
      "inlines": [
        { "type": "TextRun", "text": "Elemento: ", "weight": "Bolder" },
        { "type": "TextRun", "text": "@{outputs('Componer_-_Elemento')}" }
      ]
    },
    {
      "type": "RichTextBlock",
      "isVisible": "@{not(equals(outputs('Componer_-_Estado'), ''))}",
      "inlines": [
        { "type": "TextRun", "text": "Estado: ", "weight": "Bolder" },
        { "type": "TextRun", "text": "@{outputs('Componer_-_Estado')}" }
      ]
    },
    {
      "type": "RichTextBlock",
      "isVisible": "@{not(equals(outputs('Componer_-_Entorno'), ''))}",
      "inlines": [
        { "type": "TextRun", "text": "Entorno: ", "weight": "Bolder" },
        { "type": "TextRun", "text": "@{outputs('Componer_-_Entorno')}" }
      ]
    },
    {
      "type": "RichTextBlock",
      "isVisible": "@{not(equals(outputs('Componer_-_Grupo'), ''))}",
      "inlines": [
        { "type": "TextRun", "text": "Grupo de asignación: ", "weight": "Bolder" },
        { "type": "TextRun", "text": "@{outputs('Componer_-_Grupo')}" }
      ]
    },
    {
      "type": "TextBlock",
      "text": "📁 Detalles Adicionales",
      "wrap": true,
      "weight": "Bolder",
      "spacing": "Medium",
      "separator": true
    },
    {
      "type": "RichTextBlock",
      "isVisible": "@{not(equals(outputs('Componer_-_Tarea'), ''))}",
      "inlines": [
        { "type": "TextRun", "text": "Tarea de Catálogo: ", "weight": "Bolder" },
        { "type": "TextRun", "text": "@{outputs('Componer_-_Tarea')}" }
      ]
    },
    {
      "type": "RichTextBlock",
      "isVisible": "@{not(equals(outputs('Componer_-_Empresa'), ''))}",
      "inlines": [
        { "type": "TextRun", "text": "Empresa: ", "weight": "Bolder" },
        { "type": "TextRun", "text": "@{outputs('Componer_-_Empresa')}" }
      ]
    },
    {
      "type": "RichTextBlock",
      "isVisible": "@{not(equals(outputs('Componer_-_Solicitante'), ''))}",
      "inlines": [
        { "type": "TextRun", "text": "Solicitante: ", "weight": "Bolder" },
        { "type": "TextRun", "text": "@{outputs('Componer_-_Solicitante')}" }
      ]
    },
    {
      "type": "RichTextBlock",
      "isVisible": "@{not(equals(outputs('Componer_-_Abierto'), ''))}",
      "inlines": [
        { "type": "TextRun", "text": "Abierto: ", "weight": "Bolder" },
        { "type": "TextRun", "text": "@{outputs('Componer_-_Abierto')}" }
      ]
    },
    {
      "type": "RichTextBlock",
      "isVisible": "@{not(equals(outputs('Componer_-_Vencimiento'), ''))}",
      "inlines": [
        { "type": "TextRun", "text": "Fecha de vencimiento: ", "weight": "Bolder" },
        { "type": "TextRun", "text": "@{outputs('Componer_-_Vencimiento')}" }
      ]
    }
  ]
}
```

---

## 🧠 Resultado en Teams

La notificación se mostrará así:

```
📬 20571204 – mail

🧩 Detalles Técnicos
Elemento pedido: 17787154  
Elemento: mail
Estado: Closed Complete  
Entorno: Production  
Grupo de asignación: zzz

📁 Detalles Adicionales
Tarea de Catálogo: 20571204  
Empresa: www
Solicitante: nnn
Abierto: 08/10/2025 17:07:47 CEST  
Fecha de vencimiento: 15/10/2025 17:07:48 CEST
```

---

## ✅ Beneficios

- Control total de errores (`contains()` evita fallos).  
- Oculta líneas vacías automáticamente (`isVisible`).  
- Compatible con Adaptive Card v1.5.  
- Limpio, profesional y mantenible.

---

---

# 🧩 Mejora Power Automate – Mostrar “Tarea de Catálogo (Elemento pedido)”

## 📘 Objetivo
Actualizar el flujo existente para que en la notificación de Teams aparezca la **Tarea de Catálogo** y, entre paréntesis, el **Elemento pedido** (si existe).

---

## ⚙️ 1️⃣ Nuevo Componer – `TareaDisplay`

Crea un nuevo paso **Componer** con este nombre exacto:  
`Componer - TareaDisplay`

### Expresión PowerFx

```powerfx
if(
  equals(outputs('Componer_-_Tarea'), ''),
  '',
  if(
    equals(outputs('Componer_-_ElemPedido'), ''),
    outputs('Componer_-_Tarea'),
    concat(outputs('Componer_-_Tarea'), ' (', outputs('Componer_-_ElemPedido'), ')')
  )
)
```

### 🔍 Qué hace
| Caso | Resultado |
|:--|:--|
| Solo “Tarea” existe | `SCTASK20571204` |
| “Tarea” y “Elemento pedido” existen | `SCTASK20571204 (RITM017787154)` |
| No existe “Tarea” | *(no se muestra la línea)* |

---

## ⚙️ 2️⃣ Modificar la línea de la Adaptive Card

En la tarjeta adaptable, sustituye el bloque anterior de **Tarea de Catálogo** por este nuevo:

```json
{
  "type": "RichTextBlock",
  "isVisible": "@{not(equals(outputs('Componer_-_TareaDisplay'), ''))}",
  "inlines": [
    { "type": "TextRun", "text": "Tarea de Catálogo: ", "weight": "Bolder" },
    { "type": "TextRun", "text": "@{outputs('Componer_-_TareaDisplay')}" }
  ]
}
```

---

## 🧠 Resultado esperado en Teams

```
📁 Detalles Adicionales
Tarea de Catálogo: 20571204 (17787154)
Empresa: www
Solicitante: nnn
Abierto: 08/10/2025 17:07:47 CEST
Fecha de vencimiento: 15/10/2025 17:07:48 CEST
```





```

first(
  split(
    substring(
      variables('CuerpoLimpio'),
      indexOf(variables('CuerpoLimpio'),'Elemento pedido'),
      sub(length(variables('CuerpoLimpio')), indexOf(variables('CuerpoLimpio'),'Elemento pedido'))
    ),
    '\nElemento'
  )
)





```

---




